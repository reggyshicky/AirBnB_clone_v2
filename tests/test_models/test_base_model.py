...
@unittest.skipIf(os.getenv('HBNB_TYPE_STORAGE') == 'db',
                 'test of tebasemodel not supported')
class test_basemodel(unittest.TestCase):
    """Test class for the base_model class """

    def __init__(self, *args, **kwargs):
        """ Initializes the test class of basemode"""
    ...
    def test_kwargs_int(self):
        """Testing with specically int kwargs """
        i = self.value()
        copy = i.to_dict()
        copy.update({1: 2})
    ...
    def test_todict(self):
        ...
        # Tests to_dict datetime attr if they are strings
    ...
    def test_kwargs_none(self):
        """ Testing kwargs with None again"""
        n = {None: None}
        with self.assertRaises(TypeError):
            new = self.value(**n)
